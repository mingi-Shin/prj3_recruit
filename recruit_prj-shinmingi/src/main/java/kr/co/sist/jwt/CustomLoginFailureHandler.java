...
public class CustomLoginFailureHandler implements AuthenticationFailureHandler {
	...
	public void onAuthenticationFailure(HttpServletRequest request, HttpServletResponse response,
		...
		System.out.println("[디버깅 ]로그인 실패 이유 : ");
		System.out.println(exception.getCause());
		
		Throwable cause = exception.getCause(); //실제 원인 예외를 꺼내서 구분
		...
		if(cause instanceof UsernameNotFoundException) { //스프링 시큐리티 보안정책으로 인해 UsernameNotFoundException는 BadCredentialsException로 덮어씌워짐
			errorMsg = "등록되지 않은 이메일입니다."; //따라서 이 문구는 나올리가 없다. 
		...
		} else if (cause instanceof BadCredentialsException) { //이메일 계정 주소가 아예 등록되어 있지 않을 때.  
			errorMsg = "등록되지 않은 계정입니다. 아이디를 다시 한번 확인해주세요.";
		} else if (cause instanceof LockedException) { //이건 아직 없음 
		...
		} else if (cause instanceof CredentialsExpiredException) { //이건 아직 없어. 
			errorMsg = "비밀번호 유효기간이 만료되었습니다.";
		}
		
		// 메시지를 URL 쿼리파라미터로 인코딩해서 로그인 페이지로 리다이렉트
